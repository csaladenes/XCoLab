...
public class UserProfileBean implements Serializable {
    ...
    public UserProfileBean(User loggedInUser, Map<String, String> parameters) {
        this.loggedInUser = loggedInUser;
        this.loggedInUserId = loggedInUser != null ? loggedInUser.getUserId() : null;
        this.userIsLoggedIn = loggedInUser != null;
        ...
        if (wrappedUser != null &&
                userIsLoggedIn &&
                wrappedUser.getUserId() == loggedInUserId) {
        ...
        if (wrappedUser != null && userIsLoggedIn) {
    ...
    public void toggleEditing() throws PortalException, SystemException {
        editing = !editing;
        if (! editing) {
    ...
    //TODO: move to controller
    public void sendMessage() throws AddressException, SystemException, PortalException, MailEngineException {
        ...
        if (messageText != null && messageText.trim().length() > 0 && wrappedUser != null && userIsLoggedIn) {

            List<Long> recipients = new ArrayList<Long>();
            recipients.add(wrappedUser.getUserId());

            boolean success = MessageUtil.checkLimitAndSendMessage(messageSubject, messageText, loggedInUser,recipients);
            if (success) {
                messageSent = true;
            } else {
                limitExceeded = true;
    ...
    public boolean getSendEmailOnMessage() throws SystemException {
        MessagingUserPreferences prefs = MessageUtil.getMessagingPreferences(loggedInUserId);
    ...
    //TODO: move to controller
    public void unlinkSSOAccount() throws SystemException, PortalException {
